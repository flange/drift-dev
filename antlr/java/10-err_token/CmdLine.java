  ...
  public boolean checkCmd() {
    ...
    for (ServiceInfo si : cmdInfo.services) {
      ...
      for (String name : si.argNames) {
        ...
        synchronized(failedServices) {
          if (failedServices.contains(nameTable.get(FS_CWD + name))) {
            System.out.println("[Err] name '" + name + "' invalid due to failure\n");
            return false;
          }
        }
      ...
      public void run() {
        ...
        try {
          ...
          while (listenForFails.get()) {
            ConsumerRecords<String, byte[]> records = failConsumer.poll(1000);
